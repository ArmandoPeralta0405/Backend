...
const getAllRegister = async (req, res) => {
    try {
        ...
        const [rows] = await connection.execute('SELECT * FROM moneda');
...
const getOneRegister = async (req, res) => {
    ...
    try {
        ...
        const [rows] = await connection.execute('SELECT * FROM moneda WHERE moneda_id = ?', [ID]);
...
const insertRegister = async (req, res) => {
    try {
        ...
        const connection = await (0, database_1.conexionBD)();
        const [abreviacionRows] = await connection.execute('SELECT abreviacion FROM moneda WHERE abreviacion = ?', [abreviacion]);
        ...
        await connection.execute('INSERT INTO moneda (descripcion, abreviacion) VALUES (?, ?)', [descripcion, abreviacion]);
...
const updateRegister = async (req, res) => {
    try {
        ...
        const [monedaRows] = await connection.execute('SELECT * FROM moneda WHERE moneda_id = ?', [moneda_id]);
        ...
        if (abreviacion !== undefined && abreviacion !== monedaRows[0].abreviacion) {
            ...
            const [existingAbreviacionRows] = await connection.execute('SELECT abreviacion FROM moneda WHERE abreviacion = ? AND moneda_id <> ?', [abreviacion, moneda_id]);
        ...
        const [updatedmonedaRows] = await connection.execute('SELECT * FROM moneda WHERE moneda_id = ?', [moneda_id]);
...
const deleteRegister = async (req, res) => {
    try {
        ...
        const [deleteResult] = await connection.execute('DELETE FROM moneda WHERE moneda_id = ?', [moneda_id]);
        // Verifica si se eliminó correctamente
        if (deleteResult.affectedRows === 0) {
            return res.status(404).json({ message: 'Moneda no encontrado' });
        }
        // Crea un objeto MonedaModel
        const monedaEliminado = {
            moneda_id: deleteResult.insertId,
            descripcion: '',
