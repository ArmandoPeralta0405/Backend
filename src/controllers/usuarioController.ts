...
import { UsuarioModel } from '../models/usuarioModel'; // Importa el modelo
import { conexionBD } from '../database';
import bcrypt from 'bcrypt'; // Importa la librería de cifrado de contraseñas
...
export const getOneRegister = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [rows]: any = await connection.execute('SELECT * FROM usuario WHERE usuario_id = ?', [ID]);
...
export const insertRegister = async (req: Request, res: Response) => {
    try {
        ...
        const connection = await conexionBD();
        const [emailRows]: any = await connection.execute('SELECT email FROM usuario WHERE email = ?', [email]);
        ...
        const nuevoUsuario: UsuarioModel = {
            usuario_id: 0, // Este valor se autogenera en la base de datos
            alias,
        ...
        await connection.execute(
            'INSERT INTO usuario (alias, clave, nombre, apellido, email, cedula_identidad, fecha_registro, fecha_actualizacion, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
                nuevoUsuario.alias,
                nuevoUsuario.clave,
                nuevoUsuario.nombre,
...
export const updateRegister = async (req: Request, res: Response) => {
    try {
        ...
        const [userRows]: any = await connection.execute('SELECT * FROM usuario WHERE usuario_id = ?', [usuario_id]);
        ...
        if (email !== userRows[0].email) {
            ...
            const [existingEmailRows]: any = await connection.execute('SELECT email FROM usuario WHERE email = ? AND usuario_id <> ?', [email, usuario_id]);
        ...
        await connection.execute(
            `UPDATE usuario SET ${updateFields.join(', ')} WHERE usuario_id = ?`,
            values
        );

        // Obtén el usuario actualizado desde la base de datos
        const [updatedUserRows]: any = await connection.execute('SELECT * FROM usuario WHERE usuario_id = ?', [usuario_id]);
...
export const deleteRegister = async (req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            const [deleteResult]: any = await connection.execute('DELETE FROM usuario WHERE usuario_id = ?', [usuario_id]);
            ...
            const usuarioEliminado: UsuarioModel = {
...
export const updateEstado = async (req: Request, res: Response) => {
    try {
        ...
        const usuarioActualizado: UsuarioModel = {
            ...userRows[0], // Mantén los demás datos igual que antes
