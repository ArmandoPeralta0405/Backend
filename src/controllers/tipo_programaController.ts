...
import { TipoProgramaModel } from '../models/tipo_programaModel'; // Importa el modelo de Rol
...
export const getAllRegister = async (req: Request, res: Response) => {
    try {
        ...
        const [rows]: any = await connection.execute('SELECT * FROM tipo_programa');

        // Convierte los resultados a objetos Model
        const tipo_programas: TipoProgramaModel[] = [];
...
export const getOneRegister = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [rows]: any = await connection.execute('SELECT * FROM tipo_programa WHERE tipo_programa_id = ?', [ID]);
        ...
        const tipo_programa: TipoProgramaModel = {
...
export const insertRegister = async (req: Request, res: Response) => {
    try {
        ...
        const connection = await conexionBD();
        const [descripcionRows]: any = await connection.execute('SELECT descripcion FROM tipo_programa WHERE descripcion = ?', [descripcion]);
        ...
        await connection.execute(
            'INSERT INTO tipo_programa (descripcion) VALUES (?)',
            [descripcion]
        );
...
export const updateRegister = async (req: Request, res: Response) => {
    try {
        ...
        const [tipoprogramaRows]: any = await connection.execute('SELECT * FROM tipo_programa WHERE tipo_programa_id = ?', [tipo_programa_id]);
        ...
        if (descripcion !== tipoprogramaRows[0].descripcion) {
            ...
            const [existingDescripcionRows]: any = await connection.execute('SELECT descripcion FROM tipo_programa WHERE descripcion = ? AND tipo_programa_id <> ?', [descripcion, tipo_programa_id]);
        ...
        await connection.execute(
            'UPDATE tipo_programa SET descripcion = ? WHERE tipo_programa_id = ?',
            [descripcion, tipo_programa_id]
        );

        // Obtén el registro actualizado desde la base de datos
        const [updatedTipoProgramaRows]: any = await connection.execute('SELECT * FROM tipo_programa WHERE tipo_programa_id = ?', [tipo_programa_id]);

        // Crea un objeto Model con los datos actualizados
        const tipoprogramaActualizado: TipoProgramaModel = {
...
export const deleteRegister = async (req: Request, res: Response) => {
    try {
        ...
        const [deleteResult]: any = await connection.execute('DELETE FROM tipo_programa WHERE tipo_programa_id = ?', [tipo_programa_id]);
        ...
        const tipoProgramaEliminado: TipoProgramaModel = {
            tipo_programa_id: deleteResult.insertId, // Puedes usar el insertId como ID del registro eliminado
